...
# - install Docker only on AMD64 (ARM64 requires ARM64 kernel from host system which is not present on AMD64 host)
    case "${TARGETARCH}" in \
      *amd64*) curl -fsSL https://get.docker.com | sh ;; \
      *) groupadd --gid 990 docker ;; \
    esac && \
