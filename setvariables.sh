...
export GH_API_VERSION=$(curl -s -L -H "Accept: application/vnd.github+json" "${GH_API_URI_PREFIX}versions" | jq -r '.[0]')
...
export CONTAINER_IMAGE_NAMESPACE="stefanbosak"
export CONTAINER_IMAGE_REGISTRY="ghcr.io"
...
export CONTAINER_REPOSITORY=${CONTAINER_REPOSITORY:-"${CONTAINER_IMAGE_REGISTRY}/${CONTAINER_IMAGE_NAMESPACE}/"}
